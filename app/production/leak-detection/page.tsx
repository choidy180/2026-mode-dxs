...
const API_URL = `${API_BASE_URL}/api/DX_API000025`;
const GUIDE_IMAGE_URL = `${IMAGE_BASE_URL}/images/DX_API000102/guide_2.jpg`;

const DX_KNOWN_IMAGE_HOSTS = new Set([
  '192.168.2.147:24828',
  '1.254.24.170:24828',
...
const normalizeInspectionImageUrl = (url?: string | null, cacheKey?: string | number | null) => {
  if (!url) return '';
  ...
  // API가 images/... 처럼 leading slash 없이 내려주는 케이스 보정
...
const toCssBackgroundImage = (url?: string | null) => {
  if (!url) return 'none';
  const escapedUrl = url.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
  return `url("${escapedUrl}")`;
};

const CROP_BACKGROUND_POSITIONS: Record<CropPosition, string> = {
  'top-left': '14% 16%',
  'top-center': '50% 10%',
  'top-right': '86% 16%',
  'bottom-left': '14% 84%',
...
const usePreloadedImage = (candidateUrl: string, fallbackUrl: string) => {
  ...
  useEffect(() => {
    const nextUrl = candidateUrl || fallbackUrl;
    ...
    if (nextUrl === failedUrl && fallbackUrl) {
  ...
  }, [candidateUrl, fallbackUrl, failedUrl, retryNonce]);
...
const CamImageArea = styled.div<{ $imgUrl: string; $isGuideFallback: boolean; $bgPosition: string }>`
  flex: 1;
  position: relative;
  background-color: #F1F5F9;
  background-image: ${({ $imgUrl }) => toCssBackgroundImage($imgUrl)};
  background-size: ${({ $isGuideFallback }) => $isGuideFallback ? '280%' : 'cover'};
  background-position: ${({ $isGuideFallback, $bgPosition }) => $isGuideFallback ? $bgPosition : 'center'};
  ...
  &::after {
    ...
    inset: 0;
    pointer-events: none;
    background: linear-gradient(180deg, rgba(15,23,42,0.00), rgba(15,23,42,0.10));
  }
`;

const CamFallbackBadge = styled.span`
  position: absolute;
  left: 8px;
  bottom: 8px;
  z-index: 2;
...
const InspectionCamCard = ({
  ...
  const candidateUrl = useMemo(
    () => normalizeInspectionImageUrl(card.specificImageUrl, cacheKey),
    [card.specificImageUrl, cacheKey],
  );
  const resolvedUrl = usePreloadedImage(candidateUrl, guideImageUrl);
  const isGuideFallback = !candidateUrl || resolvedUrl === guideImageUrl;
  const previewUrl = resolvedUrl || guideImageUrl;
  const detailUrl = previewUrl;
  const isOk = card.status === '정상';

  return (
    <CamCardWrapper $isOk={isOk}>
      <CamCardHeader>
      ...
      <CamImageArea
        $imgUrl={previewUrl}
        $isGuideFallback={isGuideFallback}
        $bgPosition={CROP_BACKGROUND_POSITIONS[card.position]}
        onClick={() => onImageClick(`${card.id} - Detail View`, detailUrl)}
        title={`${card.id} 확대 이미지 보기`}
      >
        {isGuideFallback && <CamFallbackBadge>GUIDE CROP</CamFallbackBadge>}
      </CamImageArea>
    </CamCardWrapper>
  );
};
                  ...
                  <div style={{ marginTop: '32px' }}>
                    ...
                    <div
                      ...
                      style={{
                        ...
                        backgroundImage: toCssBackgroundImage(selectedLog.images.main),
                      ...
                      {[
                        ...
                        <div
                          ...
                          style={{
                            ...
                            backgroundImage: toCssBackgroundImage(selectedLog.images[corner.key as keyof typeof selectedLog.images]),
...
const ImageModal = ({ isOpen, onClose, title, imgUrl }: { isOpen: boolean, onClose: () => void, title: string, imgUrl: string }) => {
  ...
  const normalizedImgUrl = useMemo(() => normalizeInspectionImageUrl(imgUrl), [imgUrl]);

  useEffect(() => {
    if (!isOpen) return;
    setLoadFailed(false);
  }, [isOpen, normalizedImgUrl]);
      ...
      >
        ...
        <div style={{ flex: 1, backgroundColor: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
          {normalizedImgUrl && !loadFailed ? (
            ...
            <div style={{ color: '#CBD5E1', fontWeight: 800, textAlign: 'center' }}>
              이미지를 불러올 수 없습니다.<br />잠시 후 다시 시도해주세요.
            </div>
          )}
        </div>
      </div>
...
export default function VisionDashboard() {
  ...
  const guideImageUrl = useMemo(() => normalizeInspectionImageUrl(GUIDE_IMAGE_URL), []);
  const imageCacheKey = useMemo(() => {
    if (!rawApiData) return '';
    return [rawApiData.TIMEVALUE, rawApiData.COUNT_NUM].filter(Boolean).join('_');
  }, [rawApiData]);
    ...
    try {
      ...
      if (json.success) {
        if (json.data.length > 0) {
          const d = json.data[0];
          const currentCacheKey = [d.TIMEVALUE, d.COUNT_NUM].filter(Boolean).join('_');
          setRawApiData(d);
          const getStatus = (label: string): InspectionStatus => label === '정상' ? '정상' : '에러';

          setTopCards([
            { ...initialTopCards[0], status: getStatus(d.LABEL001), specificImageUrl: normalizeInspectionImageUrl(d.FILEPATH1, currentCacheKey) },
            { ...initialTopCards[1], status: getStatus(d.LABEL002), specificImageUrl: normalizeInspectionImageUrl(d.FILEPATH2, currentCacheKey) },
            { ...initialTopCards[2], status: getStatus(d.LABEL003), specificImageUrl: normalizeInspectionImageUrl(d.FILEPATH3, currentCacheKey) }
          ]);
          setBottomCards([
            { ...initialBottomCards[0], status: getStatus(d.LABEL006), specificImageUrl: normalizeInspectionImageUrl(d.FILEPATH6, currentCacheKey) },
            { ...initialBottomCards[1], status: getStatus(d.LABEL005), specificImageUrl: normalizeInspectionImageUrl(d.FILEPATH5, currentCacheKey) },
            { ...initialBottomCards[2], status: getStatus(d.LABEL004), specificImageUrl: normalizeInspectionImageUrl(d.FILEPATH4, currentCacheKey) }
          ]);
  ...
  const handleImageClick = (title: string, url: string) => {
    setModalInfo({
      isOpen: true,
      title,
      imgUrl: normalizeInspectionImageUrl(url),
    });
  };
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              backgroundImage: toCssBackgroundImage(guideImageUrl),
            }}
          >
