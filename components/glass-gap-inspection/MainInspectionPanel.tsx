...
import CameraTile from '@/components/glass-gap-inspection/CameraTile';
import GuideViewport from '@/components/glass-gap-inspection/GuideViewport';
import BoxConnectors from '@/components/glass-gap-inspection/BoxConnectors';
import {
  CameraRail,
  CameraStackGrid,
  InspectionWorkspace,
    ...
    <InspectionWorkspace>
      <MainInspectionPanelShell>
        ...
        <MainImageStage ref={stageRef}>
          {viewType === 'split' ? (
            <>
              <StageSplitGrid>
                <CameraRail>
                  {leftCameraItems.map(renderCameraTile)}
                </CameraRail>

                {renderGuideViewport(false)}

                <CameraRail>
                  {rightCameraItems.map(renderCameraTile)}
                </CameraRail>
              </StageSplitGrid>
            ...
            <>
              <StageRightStackGrid>
                ...
                <CameraStackGrid>
                  {cornerItems.map(renderCameraTile)}
                </CameraStackGrid>
              </StageRightStackGrid>
        ...
        <MainPanelFooter>
          ...
          <span>모서리 버튼을 드래그하면 확대 위치와 카메라 확대 초점이 함께 변경됩니다.</span>
        </MainPanelFooter>
      </MainInspectionPanelShell>
    </InspectionWorkspace>
  );
